...
class LavalinkPlayer(wavelink.Player):
    ...
    def setup_hints(self):
        ...
        if self.static:
            hints.append("Bạn có thể sửa chữa danh sách nhạc/phát trong tin nhắn người chơi khi bạn có "
                         " để cho phép các thành viên nghe họ công khai. Để sử dụng "
    ...
    async def members_timeout(self, check: bool, force: bool = False):
        ...
        if check:
            ...
            if update_log:
                self.set_command_log(emoji="🔰", text="Bài hát đã được tiếp tục phát (**Tự động**)")
                if self.current:
        ...
        if self.keep_connected:
            ...
            self.set_command_log(text=f"Người chơi bị tạm dừng vì thiếu thành viên trên kênh. Và "
                                      f"Bài hát sẽ được tiếp tục tự động khi một thành viên vào kênh "
