...
def run_lavalink(
    ...
    print(f"Khởi động máy chủ Lavalink (tùy thuộc vào máy chủ lưu trữ, Lavalink có thể mất một lúc để khởi động, "
          f"có thể thất bại trong một số lần thử kết nối cho đến khi nó bắt đầu hoàn toàn).\n{'-' * 30}")

    lavalink_process = subprocess.Popen(java_cmd.split(), stdout=subprocess.DEVNULL)

    if lavalink_additional_sleep:
        print(f"Vui lòng chờ {lavalink_additional_sleep} giây...\n{'-' * 30}")
