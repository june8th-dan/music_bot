        ...
        for guild_id in list(self.bot.music.players):
            ...
            try:
                ...
                if not player.is_paused and not player.is_playing:
                    await player.process_next()
                print(f"{self.bot.user} - {player.guild.name} [{guild_id}] - Người chơi kết nối lại và không có kênh giọng nói")
            except:
                traceback.print_exc()
        ...
        try:
            ...
            if control == PlayerControls.enqueue_fav:
                ...
                try:
                    ...
                    raise GenericError(
                        "**Bạn có một tương tác chưa thực hiện!**\n`Nếu đó là thư ẩn, hãy tránh nhấp vào \"bỏ qua\".`")
                ...
                if control == PlayerControls.add_favorite:
                    ...
                    if len(choices) == 1:
                        ...
                        await interaction.send(
                            embed=disnake.Embed(
                                color=self.bot.get_color(interaction.guild.me),
                                description=f"### Chọn một mục từ bài hát hiện tại để thêm vào mục yêu thích của bạn:\n\n{msg}"
                    ...
                    if self.bot.config["MAX_USER_FAVS"] > 0 and not (await self.bot.is_owner(interaction.author)):
                        ...
                        if len(user_data["fav_links"]) >= self.bot.config["MAX_USER_FAVS"]:
                            await interaction.response.edit_message(
                                embed=disnake.Embed(
                                    color=self.bot.get_color(interaction.guild.me),
                                    description="Bạn không có đủ dung lượng để thêm tất cả dấu trang từ tệp của mình...\n"
                    ...
                    await interaction.edit_original_response(
                        embed=disnake.Embed(
                            color=self.bot.get_color(interaction.guild.me),
                            description="### Mục được thêm/chỉnh sửa thành công vào mục yêu thích của bạn:\n\n"
