...
class Misc(commands.Cog):
    ...
    async def about(
        ...
        embed.description += f"> <:KleeParty:1136214064067006565> **Số máy chủ mà tôi đang ở" + (" (Tất cả các bot)" if guilds_size > 1 else "") + \
                             f":** `{guilds_size}`\n"
        ...
        if active_players_other_bots:
            embed.description += f"> <a:awbw_music:1138360100529127465> **Đang phát" + (" (Tất cả các bot)" if len(bot.pool.bots) > 1 else "") + \
                                 f":** `{active_players_other_bots}`\n"

        if paused_players_other_bots:
            embed.description += f">  <a:awbw_music:1138360100529127465> **Tạm dừng" + (" (Tất cả các bot)" if len(bot.pool.bots) > 1 else "") + \
                                 f":** `{paused_players_other_bots}`\n"

        if inactive_players_other_bots:
            embed.description += f"> <a:awbw_music:1138360100529127465> **Không hoạt động" + (" (Tất cả các bot)" if len(bot.pool.bots) > 1 else "") + \
                                 f":** `{inactive_players_other_bots}`\n"

        embed.description += f"> <:python:1119837192152424448> **Phiên bản Python:** `{platform.python_version()}`\n" \
                             f"> <:disnake:1119830259756454019> **Phiên bản disnake:** `{disnake.__version__}`\n" \
                             f"> <a:loading:1117802386333905017> **Độ trễ:** `{round(bot.latency * 100)}ms`\n" \
                             f"> <:ram:1138360456399028254> **Mức độ sử dụng Ram** `{ram_usage}`\n" \
                             f"> <a:aloadig:1138360673068384366> **Lần khởi động lại cuối cùng:** <t:{int(bot.uptime.timestamp())}:R>\n"
